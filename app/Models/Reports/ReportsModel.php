...
Class ReportsModel extends MasterModel
{
    ...
    public function generateReportOrdersWithCompanyAndRol(int $IdCompany = null, int $rol_id = null, $date_init, $date_end)
    {
        $sql    = "SELECT DISTINCT
                        ...
                        `orders`.order_total,
                        `orders`.order_iva,
                        extra_attributes_order.order_attrs_name,
                        extra_attributes_order.order_attrs_desc,
                        company.*, users.*,
                        (SELECT SUM(order_subtotal) FROM `orders`) AS total_subtotal,
                        (SELECT SUM(order_total) FROM `orders`) AS total_total,
                        (SELECT SUM(order_iva) FROM `orders`) AS total_iva
                    FROM
                        `orders`
                        INNER JOIN users ON users.u_id = `orders`.u_id
                        INNER JOIN company ON company.c_id = users.c_id
                        INNER JOIN order_articles ON order_articles.order_id = `orders`.order_id
                        LEFT JOIN extra_attributes_order ON extra_attributes_order.order_id = `orders`.order_id
                    WHERE
                        `orders`.order_date BETWEEN :date_init AND :date_end";
